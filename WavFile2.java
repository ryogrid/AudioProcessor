...
public class WavFile2
{
	...
	public static WavFile2 newWavFile(File file, int numChannels, long numFrames, int validBits, long sampleRate) throws IOException, WavFileException
	{
		// Instantiate new Wavfile and initialise
		WavFile2 wavFile = new WavFile2();
		wavFile.file = file;
	...
	public static WavFile2 openWavFile(File file) throws IOException, WavFileException
	{
	...
	public int writeFrames(double[] sampleBuffer, int offset, int numFramesToWrite) throws IOException, WavFileException
	{
		...
		for (int f=0 ; f<numFramesToWrite ; f++)
		{
			...
			for (int c=0 ; c<numChannels ; c++)
			{
				byte[] tmp = writeSample((long) (floatScale * (floatOffset + sampleBuffer[offset])));
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			{
				WavFile2 readWavFile = openWavFile(new File(filename));
				readWavFile.display();
				...
				WavFile2 writeWavFile = newWavFile(new File("out.wav"), numChannels, numFrames, validBits, sampleRate);
...
//				long[] buffer = new long[BUF_SIZE * numChannels];
			...
			WavFile2 writeWavFile = newWavFile(new File("out2.wav"), 1, 10, 23, 44100);
